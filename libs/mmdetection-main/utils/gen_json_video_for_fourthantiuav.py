...
adenomatous_json_dir = 'D:/Codes/Datasets/Track1/train/'+mode
image_root = ''
dataset_root = ''

# out_path='datasets/annotationsfourthantiuav/'
out_path='D:/Codes/Datasets/Track1/train/val_annotations/'
...
while True:
    ...
    for idx, f in tqdm(enumerate(os.listdir(adenomatous_json_dir))):
        ...
        if os.path.isfile(os.path.join(adenomatous_json_dir, f)):
            continue
        ...
        with open(json_dir) as json_file:
            ...
            exist = data['exist']
            annos = data['gt_rect']
            images = glob.glob(os.path.join(adenomatous_json_dir, f, '*.*g'))
            file_name, file_extension = os.path.splitext(images[0])
            ...
            if myid > len(annos)-1:
                ...
                img['file_name'] = osp.join(f, str(myid + 1).zfill(6) + file_extension)
                img_ori = cv2.imread(osp.join(adenomatous_json_dir, img['file_name']))
