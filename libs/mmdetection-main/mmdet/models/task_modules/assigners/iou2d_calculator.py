...
class BboxOverlaps2D_GLIP(BboxOverlaps2D):
    ...
    def __call__(self, bboxes1, bboxes2, mode='iou', is_aligned=False):
        ...
        if bboxes1.size(0) == 0 or bboxes2.size(0) == 0:
            if is_aligned:
                return torch.zeros(bboxes1.size(0), dtype=bboxes1.dtype, device=bboxes1.device)
            else:
                # 确保返回正确的形状
                if bboxes1.size(0) == 0:
                    return torch.zeros(0, bboxes2.size(0), dtype=bboxes1.dtype, device=bboxes1.device)
                else:
                    return torch.zeros(bboxes1.size(0), 0, dtype=bboxes1.dtype, device=bboxes1.device)
        
        # 检查边界框的维度
        if bboxes1.dim() != 2 or bboxes2.dim() != 2:
            raise ValueError(f"Expected 2D tensors, got bboxes1.dim()={bboxes1.dim()}, bboxes2.dim()={bboxes2.dim()}")
