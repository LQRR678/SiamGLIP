...
class ATSSVLFusionHead(ATSSHead):
    ...
    def _loss_by_feat(self, anchors: Tensor, cls_score: Tensor,
        ...
        text_mask = (self.text_masks > 0)
        target_len = min(text_mask.size(-1), cls_score.size(-1))
        text_mask = text_mask[:, :target_len].unsqueeze(1)
        # align cls_score/labels/label_weights to target_len
        if cls_score.size(-1) > target_len:
            cls_score = cls_score[..., :target_len]
        if labels.size(-1) > target_len:
            labels = labels[..., :target_len]
        elif labels.size(-1) < target_len:
            pad_len = target_len - labels.size(-1)
            labels = torch.cat(
                [labels, labels.new_zeros(labels.shape[0], labels.shape[1], pad_len)],
                dim=-1)
        label_weights = label_weights[..., None].repeat(1, 1, target_len)
